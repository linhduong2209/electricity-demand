...
\section{Referencing and Hyperlinking within the Document}
...
\textbf{To change between referencing styles, go to line 21 of thesis.tex where it says \%\%\%\% REFERENCING STYLES \%\%\%\%, and comment/uncomment accordingly!} The following subsections will explain more about both styles (NOTE: This pdf has been compiled using the Harvard referencing style, as it is the one preferred by the University). More information about referencing according to RGU guidelines can be found here:

\begin{enumerate}
    \item \url{https://library.rgu.ac.uk/referencing-updates-staff}
    \item \url{https://library.rgu.ac.uk/referencing-and-refworks}
\end{enumerate}

\subsection{The Harvard Bibliography Style}

The bibliographic references in this document laid out using the Harvard style. Further information about this style may be found at the following links: 

\begin{enumerate}
    \item \url{https://library.rgu.ac.uk/RGUHarvard}
    \item \url{https://library.rgu.ac.uk/harvard-referencing-templates}
    \item \url{https://library.rgu.ac.uk/ctr-harvard-referencing-templates}
\end{enumerate}

If you want parenthesis to enclose the reference, use \textit{parencite} or \textit{autocite} instead of \textit{cite}. This would change the citation style in the previous paragraph from:

\cite{book:einsteinBrownianMovement} for example is well known for his work on Brownian Movement.
...
Most readers recommend using the Vancouver citation style, as it enhances readability and allows you to refer to papers through a number within squared brackets [x] rather than using last names and years within the text, which can sometimes be confusing. From 1st September 2024 onwards, RGU has decided also to allow this style as mentioned in the following links: 

\begin{enumerate}
    \item \url{https://library.rgu.ac.uk/ctr-vancouver-referencing-templates}
\end{enumerate}

\subsection{Compiling a BibTeX Database}

Having initially compiled the document using pdfLatex a number of helper files are created that aid in referencing and citations. One then must compile the bibtex database, followed by an additional two compiles using pdfLatex. Citing additional bibliographic references within the body of the document being produced will require the recompile of the bibtex database. In the case that the bibtex reference of a cited article cannot be found one will see \textbf{a text in bold} (in case of using Harvard) or a question mark inside squared brackets \textbf{[?]} (in case of using Vancouver) instead of the proper citation.
